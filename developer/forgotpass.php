			...
			<div class="container" style="margin-top: 35px;">
						...
						<div class="section-title" style="justify-content: center;display: flex;">
							<?php
								...
								$select = conectar2("SELECT * FROM usu_usuarios WHERE email = :email_usuario AND remember_token = :token");
							    $select->execute(array(':email_usuario' => $email, ':token' => $token));
							    $resultadoUs = $select->fetch();
							    
							    if ($select->rowCount() > 0) {

									    $updateUs = "UPDATE usu_usuarios SET remember_token = :remember_token WHERE email = :email_usuario";
										$con = conectar2($updateUs);

  
									    // if ($con->execute(array(':email_usuario' => $email, ':remember_token' => ''))) {
